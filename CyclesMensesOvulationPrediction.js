...
async function load(model) {
  ...
  const modelInfo = models["file://./model"];
  if (modelInfo) {
...
async function modelTrainingAndPredictingOnDataset(data, timePortion) {
  ...
  const tensorTrainX = await tf
    .tensor1d(trainX)
    .reshape([size - timePortion, timePortion, 1]);
  ...
  return predictNextMonthValue(
    generateNextDayPrediction(features, timePortion),
    model,
    min,
    max,
    timePortion
...
async function predictNextMonthValue(
  predictionInput,
  model,
  min,
  max,
  ...
  let tensorNextMonth = tf
    .tensor1d(nextMonth.data)
    .reshape([1, timePortion, 1]);
  ...
  var inversePredictedValue = minMaxInverseScaler(
    predictedValue.dataSync()[0],
    min,
    max
  );
...
async function predictAllValues(userId) {
  ...
  // we initialize 3 arrays for eache features that we will use to train and predict the 3 next values
  const cyclelengtharray = Array.from(Array(cycles.length), () => new Array(1));
  const menseslengtharray = Array.from(
    Array(cycles.length),
    () => new Array(1)
  );
  const ovulationdayarray = Array.from(
    Array(cycles.length),
    () => new Array(1)
  );
  ...
  for (i = 0; i < cycles.length; i++) {
    ...
    var eggStart = new Date(cycles[i]["eggStart"]);
    var eggEnd = new Date(cycles[i]["eggEnd"]);
    //then we calculate the legnthOfCycle, lengthOfPeriods and ovulationDay of each previous cycles
    var lengthOfCycle = Math.round(
      (eggEnd.getTime() - bleedStart.getTime()) / (1000 * 3600 * 24)
    );
    var lengthOfPeriods = Math.round(
      (bleedEnd.getTime() - bleedStart.getTime()) / (1000 * 3600 * 24)
    );
    var ovulationDay = Math.round(
      (eggStart.getTime() - bleedStart.getTime()) / (1000 * 3600 * 24)
    );
    // we then fill the arrays witht the values calculated
    cyclelengtharray[i][0] = lengthOfCycle;
    menseslengtharray[i][0] = lengthOfPeriods;
    ovulationdayarray[i][0] = ovulationDay;
  }

  // we use it to predict the value of the next cycleLength
  var nextCycleLength = await modelTrainingAndPredictingOnDataset(
    cyclelengtharray,
    timePortion
  );

  // we use it to predict the value of the next cycleLength
  var nextMensesLength = await modelTrainingAndPredictingOnDataset(
    menseslengtharray,
    timePortion
  );

  // we use it to predict the value of the next cycleLength
  var nextOvulationDay = await modelTrainingAndPredictingOnDataset(
    ovulationdayarray,
    timePortion
  );
  console.log(nextCycleLength, nextMensesLength, nextOvulationDay);
  ...
  var predictedBleedStart = new Date(Number(eggEnd));
  var predictedBleedEnd = new Date(Number(eggEnd));
  var predictedEggStart = new Date(Number(eggEnd));
  var predictedEggEnd = new Date(Number(eggEnd));
  predictedBleedStart.setDate(predictedBleedStart.getDate() + 1);
  predictedBleedEnd.setDate(predictedBleedEnd.getDate() + nextMensesLength + 1);
  predictedEggStart.setDate(predictedEggStart.getDate() + nextOvulationDay + 1);
  predictedEggEnd.setDate(predictedEggEnd.getDate() + nextCycleLength + 1);
