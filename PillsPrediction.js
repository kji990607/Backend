//we start by requiring the tensorflow module
...
//function to save a model, we should only have to use it once
...
async function load(model) {
  ...
  return (model = await tf.loadLayersModel(
    "file://./pillpredictionmodel/model.json"
  ));
}
...
async function modelTraining(points) {
  ...
  const labelTensor = tf.tidy(() =>
    tf.oneHot(tf.tensor1d(labelValues, "int32"), 12)
  );
...
async function predict(predictionInput) {
  ...
  const outputValue = getClassName(
    tf.argMax(model.predict(inputTensor), (axis = 1))
  );
  return output;
}
...
function getClassIndex(className) {
  if (className == 1 || className === "1") {
    ...
    return 1;
  } else if (className == 3 || className === "3") {
    return 2;
  } else if (className == 4 || className === "4") {
    return 3;
  } else if (className == 5 || className === "5") {
    return 4;
  } else if (className == 6 || className === "6") {
    return 5;
  } else if (className == 7 || className === "7") {
    return 6;
  } else if (className == 8 || className === "8") {
    return 7;
  } else if (className == 9 || className === "9") {
    return 8;
...
async function modelPredictingOnUserDataset() {
  ...
  var userRow = [];
  for (var i = 0; i < 14; i++) {
    userRow.push(input[0][i]);
  }
  model = await load(model);
  var pill = await predict(userRow);
  console.log(pill);
}
//modelTrainingOnDataset();
modelPredictingOnUserDataset();
